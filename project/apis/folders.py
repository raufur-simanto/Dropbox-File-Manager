...
create_folder_model = folder_namespace.model("create_folder_model", {
...
class CreateFolder(Resource):
    ...
    def post(self):
        ...
        response = requests.post('https://api.dropboxapi.com/2/files/create_folder_v2', headers=headers, json=data)
        
        if response.status_code != 200:
            app.logger.info(f"response status: {response.status_code}, response: {response.json()}")
