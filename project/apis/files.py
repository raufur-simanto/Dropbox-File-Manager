...
file_model = file_namespace.model('File', {
    ...
    'path_lower': fields.String(required=True, description='The lowercase path of the file'),
    'id': fields.String(required=True, description='The file ID'),
    'size': fields.Integer(required=True, description='The size of the file in bytes')
})
file_list_model = file_namespace.model('file_list_model', {
    "files": fields.Nested(file_model)
})

search_file_model = file_namespace.model('SearchFile', {
    'file_name': fields.String(required=True, description='The name of the file to search for'),
    'path': fields.String(required=False, description='The path of the file to search for')
})

rename_file_model = file_namespace.model('RenameFile', {
    'current_name': fields.String(required=True, description='The previous name of the file'),
    'new_name': fields.String(required=True, description='The new name of the file'),
...
class SearchFiles(Resource):
    ...
    def post(self):
        ...
        response = requests.post('https://api.dropboxapi.com/2/files/search_v2', headers=headers, json=data)
...
class UploadFile(Resource):
    def post(self):
        ...
        if path:
            ...
            arg_value = '{"path": "/' + file.filename + '","mode": "add","autorename": true,"mute": false}'
        ...
        response = requests.post('https://content.dropboxapi.com/2/files/upload', headers=headers, data=file)
...
class RenameFile(Resource):
    ...
    def put(self):
        ...
        response = requests.post('https://api.dropboxapi.com/2/files/move_v2', headers=headers, json=payload)
        app.logger.info(f"response:: {response.json()}")
